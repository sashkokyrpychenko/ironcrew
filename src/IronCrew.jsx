...
    // Для кожного профілю — останнє повідомлення
...
          ini: (p.name || "??").slice(0, 2).toUpperCase(),

          col: "#666",

          online: false,

          prev: msgs?.[0]?.content || "Почати розмову",

          time: msgs?.[0]?.created_at

            ? new Date(msgs[0].created_at).toLocaleTimeString("uk", { hour: "2-digit", minute: "2-digit" })
...
          {realMsgs.length===0&&<div style={{textAlign:"center",color:"var(--muted)",fontSize:13,marginTop:40}}>Почни розмову 💬</div>}
...
          {(realChats.length > 0 ? realChats : chatsData).map(c=>(
